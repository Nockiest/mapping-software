...
export type MarkerProps = {
  ...
  customSettings?: Record<string, any>; // customSettings is an object with any properties
}
...
const Marker: React.FC<MarkerProps> = ({
  ...
  const [initialMarkerSettings] = useState({
  ...
  const markerStyle: React.CSSProperties = {
    ...
    width: `${usedSettings.width ?? 25}px`, // Default width is 50 if usedSettings.width is null or undefined
    color: `${usedSettings.textColor ?? 'black'} `,
    height: `${usedSettings.width ?? 25}px`,
    fontSize: `${(usedSettings.width ?? 50) / 4}px`,
    backgroundColor: usedSettings.color ?? 'lightgray', // Default color is lightgray if usedSettings.color is null or undefined
